...
- 项目定义与阶段总览：`docs/00_文档总目录_V1.1.md`
- 产品范围与用户体验：`docs/01_产品需求文档_PRD_V1.1.md`
- 技术架构与模块边界：`docs/02_技术方案设计_TDD_V1.1.md`
- 数据语义与实体设计：`docs/03_数据架构与数据库设计_DDD_V1.1.md`
- API 契约与外部集成：`docs/04_API与外部集成方案_V1.1.md`
- 阶段计划与依赖：`docs/05_研发计划与WBS_V1.1.md`
- 里程碑与验收门禁：`docs/06_里程碑与验收标准_V1.1.md`
- 测试、发布与运维：`docs/07_测试发布与运维方案_V1.1.md`
- 已批准架构决策与风险：`docs/08_风险清单与ADR_V1.1.md`
...
- MVP 优化方案：`docs/19_MVP优化方案_V1.0.md`
- 后端 Node 技术栈迁移候选评估：`docs/20_后端Node技术栈迁移评估与任务清单_V1.0.md`（`PROPOSED / NOT APPROVED`，不得视为已批准架构）
...
- 候选评估文档不能覆盖已批准 ADR；Node 后端迁移只有在负责人批准并新增正式 ADR 后才能实施。
- 文件名和正文版本必须同步升级；引用时写明文件路径与章节。
...
每完成一次独立的修复或变更，必须立即提交并推送到远程仓库，不得多个修复攒成一次提交。

1. 一次提交只承载一个可独立回滚的变更。修复、重构和格式化不得混在同一次提交。
2. 提交前必须先跑通受影响范围的格式化、静态检查和测试，不提交已知失败的代码。
3. 提交信息使用中文，说明「问题是什么、为什么这样修、验证到什么程度」，而不是罗列改了哪些文件。
4. 提交后立即 `git push` 到远程对应分支，不在本地积压。
5. 日常开发在 `develop` 分支进行，`main` 只接受经过验收的合并。
6. 若某次修复暴露出计划外的问题，应单独成一次提交并在信息中写明是如何被发现的。

该规则的目的是让每一条修复都可被独立 Review 与回滚。攒批提交会让「哪一处改动引入了回归」变得不可追溯。

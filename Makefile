.PHONY: install backend-install frontend-install dev-api dev-miniapp worker beat test lint typecheck structure-check docs-check security-audit build infra-up infra-observability-up infra-down migrate terraform-staging-validate terraform-staging-audit-state terraform-staging-plan local-db-drill local-alert-drill staging-alert-drill local-api-baseline staging-ai-capacity staging-queue-recovery event-funnel-audit staging-smoke staging-security-audit
...
terraform-staging-audit-state:
	./scripts/infra.sh terraform-staging-audit-state

terraform-staging-plan:
	./scripts/infra.sh terraform-staging-plan
